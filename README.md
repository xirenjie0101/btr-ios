...
- **真机上跑过一轮（1.0.0）**：MITM、脚本、多线程取回都能工作，字节没出过错。但真机统计也说明了 1.0 的思路不对——官方 App 的播放器是**一小段一小段（约 1 MB）地要，上一段到手才要下一段**；只把这一小段拆开并行去取，每个连接都要重新握手、慢启动，反而比 App 自己那条热连接更慢（平均 323 KB/s，比不装还卡）。1.1 改成了“预读”，1.2 又按真机测到的节点速度重新分配连接（见下文原理），效果还等验证：**装上以后看一会儿视频，把统计页截图发回来。**
...
| 参数 | 默认 | 说明 |
| --- | --- | --- |
| 并发连接数 | 32 | 每一批同时开几条连接。到大陆节点单条连接很慢，靠的是数量；节点之间差别极大（真机测到 12 KB/s 到 269 KB/s 都有），快的节点会分到更多、更大的块。发热明显就调到 16，缓冲还跟不上可以试 64 |
| 预读大小 | 4MB | App 要一小段时顺便多取多少放进缓存，接下来的几段就直接命中。每一批实际能取多少由各节点测得的速度决定（每条连接约 1 秒能拿回来的量），这里是上限；小文件（音频）按文件长度的 1/8 封顶。“关闭”＝只取 App 要的那一段（1.0 的行为） |
| 缓存上限 | 16MB | 预读数据最多占多少存储（一批预读最多用掉一半）；播放位置之前的、超过 10 分钟的自动淘汰。如果 Loon 变得不稳定（VPN 自己断开），调到 8MB |
...
| 手机发热、耗电明显 | 连接数调到 16；这是 MITM 解密视频流量的固有开销 |
| 统计页“缓存”一行显示“已停用” | 存储自检没通过或写不进去：Loon 的持久化存储装不下这么多数据。把缓存上限调小再看；实在不行就“关闭”，退回 1.0 的行为 |
| Loon 的 VPN 自己断开 | 很可能是内存超了（网络扩展的内存有限）：缓存上限调到 8MB、预读调到 2MB、连接数调到 16 |
...
App：Range: bytes=a-b（约 1 MB）
   ...
   1.1+ → 缓存里有 [a,b]？有 → 直接答复（几毫秒）
          没有 → 一批并行取回 [a,b] 和后面的几 MB（预读）：每条连接分到“它的节点约 1 秒能送到的量”
               （真机上节点之间差 20 倍，平均分会让整批等最慢的那条；这样分，整批差不多同时到；
               预读块切得比 App 要的块略短，先到先存，答复不必等它们）
               → [a,b] 交给最快的几条连接，其余连接预读；取回的全部 base64 后存进 $persistentStore
               答复控制在 1 秒多一点：真机统计显示 App 等不到答复就会重发同一段，宁可一批少取一点
          节点速度按每个子块实测持续更新；没测过的节点每批只分到一两个小预读块；一直不答的节点会被停用
          文件长度还不知道时（刚开始播）预读只取 1 MB，先把开头送到
...
缓存：每条记录是一段连续字节（最多 1 MB 一条）；超过上限时先淘汰播放位置之前 512 KB 以外的、再淘汰最旧的；10 分钟没用也淘汰。第一次用会做一次存储自检（写 1 MB 再读回来），没通过就自动不用缓存（只做多线程，等于把预读关掉）。

其他安全网：App 要的每个子块最多换 4 个节点；剩下一两个慢块时会向刚证明可用的节点再要一份（对冲）；预读子块只等到它该到的时间，没到的丢掉，迟迟不答的节点记一次“迟到”（速度估计下调，连续三次停用）；请求出错的节点连续两次停用 5 → 15 → 60 分钟；8 秒内拼不齐就原样放行；同一来源连续 4 次失败就整体放行 5 分钟；网页发出的请求（带 Origin）一律不碰。

1.1.0 在真机上做过的两个实验说明：Loon 在脚本 `$done` 之后立刻结束脚本（定时器、网络回调都不再执行），脚本自己发的请求也不会再触发脚本。所以“答复之后在后台继续预读”做不到，预读只能和 App 要的那一段放在同一批里取。
...
- **1.3.0**（2026-09-18）答复时间压到约 1 秒（每条连接的块按 1 秒切，预读块略短先到，答复不等预读；超过 2 MB 的请求按一批的容量分块答复）；画面和声音两路同时取流时共享每个节点的连接额度；小文件的预读按文件长度 1/8 封顶，不让音频把缓存挤满；统计页增加“最近 12 次”逐条记录、App 重问之前那次答复的耗时区间（推断 App 的耐心）、MCDN/PCDN 取流请求计数。
- **1.2.0**（2026-09-18）真机统计（节点速度 12～269 KB/s 不等、8 条连接、大范围请求 14 秒超时）之后的重做：每条连接按其节点实测速度分块，快的节点承担大头，慢的节点不再拖住整批；预读量不再依赖线程数（1.1.0 里 8 线程时预读会算成 0）；一批不超过缓存上限的一半；取回的 App 数据也进缓存（重试、回退直接命中）；答复超时从 14 秒降到 8 秒；“迟到”与“出错”分开计数，快节点不再因偶尔迟到被停用；统计页增加重复位置的分类、最大请求、超过 4 秒的答复数、节点迟到次数。参数“并发线程”改名“并发连接数”，默认 32；缓存上限默认 16MB。
