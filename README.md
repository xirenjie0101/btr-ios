...
App：Range: bytes=a-b（约 1 MB）
   ...
   1.1+ → 缓存里有 [a,b]？有 → 直接答复（几毫秒）
          ...
          带宽是所有节点之和：每批先给每个可用节点各分一条连接（约 20 个候选，大陆 + 香港/海外一起用），
               快的节点再多分几条、承担 App 要的那一段；没测过的节点每批探测几个，很快摸清一大批
          节点速度按每个子块实测持续更新；只有连续真正出错的节点才会被停用，单纯慢（迟到）绝不停用
...
其他安全网：App 要的每个子块最多换 4 个节点；剩下一两个慢块时会向刚证明可用的节点再要一份（对冲）；预读子块只等到它该到的时间，没到的丢掉；迟迟不答的节点记一次“迟到”，但只作记录——不下调速度、不停用（海外高峰期几乎每个节点都会迟到，停用只会让总带宽更少）；只有连续三次真正出错（取不到 / 403 / 坏数据）的节点才停用 3 → 9 → 30 分钟，一次成功即恢复；8 秒内拼不齐就原样放行；同一来源连续 4 次失败就整体放行 5 分钟；网页发出的请求（带 Origin）一律不碰。
...
- **1.4.0**（2026-09-18）针对“每个节点都很慢”的海外高峰期重做取流：**节点池从 8 个扩到约 20 个**（大陆镜像更全，并把香港/海外节点也一起纳入候选，无论 CDN 选的是哪边），带宽按“所有节点相加”来算——每批先给每个可用节点各分一条连接、快的再多分。修掉两个把慢网络越拖越慢的老问题：① 单条连接最小块从 32KB 降到 8KB，以前节点只有几十 KB/s 时每条连接都算不到 32KB、整批容量被算成 0（真机统计里的“容量 0 KB”）；② “迟到”不再下调节点速度、也不再据此停用节点——高峰期几乎每个节点都会迟到，之前会把最快的节点（如 ali）自己停掉，越用越少；现在只有连续三次真正出错才停用，且更温和（3→9→30 分钟，一次成功即恢复）。统计页“节点”栏顶部新增“在用 N 个镜像·合计约 X KB/s”，一眼看出海外链路的总带宽。
