...
| 预读大小 | 4MB | App 要一小段时顺便多取多少放进缓存，接下来的几段就直接命中。每一批实际能取多少由各节点测得的速度决定（每条连接 1.5 秒能拿回来的量），这里是上限。“关闭”＝只取 App 要的那一段（1.0 的行为） |
...
App：Range: bytes=a-b（约 1 MB）
   ...
   1.1+ → 缓存里有 [a,b]？有 → 直接答复（几毫秒）
          没有 → 一批并行取回 [a,b] 和后面的几 MB（预读）：每条连接分到“它的节点 1.5 秒能送到的量”
